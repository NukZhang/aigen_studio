...
public class FrontendScaffoldService {
    ...
    public void ensureVueScaffoldAndInjectPrototype(Path frontendDir, String uiPrototypeHtml, String projectName) {
        ...
        try {
            ...
            ensureAppVue(srcDir, uiPrototypeHtml);
        } catch (Exception e) {
    ...
    private void ensureAppVue(Path srcDir, String uiPrototypeHtml) throws Exception {
        Path appVue = srcDir.resolve("App.vue");
        if (Files.exists(appVue)) {
            String existing = Files.readString(appVue);
            if (!existing.isBlank()) {
                log.info("Keeping existing App.vue generated by AI: {}", appVue);
                return;
            }
        }
